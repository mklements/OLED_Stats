...
def update():
    confirm = typer.confirm(
        "This will update all software to current version, with no changes to companion file. OK?"
    )
    if not confirm:
        print("Aborting...")
        raise typer.Abort()
...
def factory():
    confirm = typer.confirm(
        "This will factory reset device you will lose all companion settings, OK?"
    )
    if not confirm:
        print("Aborting...")
        raise typer.Abort()
